...
// Hardcoded product data to act as a single source of truth for the application.
// This replaces getting data from the DOM attributes.
const allProducts = [
  {
    id: 1,
    name: 'Black Bagru Hand Block Printed Off Sleeves Kurta',
    image: 'https://placehold.co/600x800/e9e7e5/000?text=Product+Image+1',
    price: 2099,
    size: 'S',
  },
  {
    id: 2,
    name: 'Black Bagru Hand Block Printed Kurta',
    image: 'https://placehold.co/600x800/e9e7e5/000?text=Product+Image+2',
    price: 2049,
    size: 'M',
  },
  {
    id: 3,
    name: 'Brown Jariota Hand Block Printed Cotton Kurta',
    image: 'https://placehold.co/600x800/e9e7e5/000?text=Product+Image+3',
    price: 1999,
    size: 'L',
  },
  {
    id: 4,
    name: 'New Product XL',
    image: 'https://placehold.co/600x800/e9e7e5/000?text=Product+Image+4',
    price: 2500,
    size: 'XL',
  },
  {
    id: 5,
    name: 'New Product M',
    image: 'https://placehold.co/600x800/e9e7e5/000?text=Product+Image+5',
    price: 1500,
    size: 'M',
  },
];
...
export default function App() {
  ...
  const sortedProducts = React.useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'featured':
      default:
        // No sorting, or could implement a custom "featured" logic
        break;
    }
    return products;
  }, [filteredProducts, sortBy]);

  // Handles opening the product quick view sidebar.
  const handleOpenSidebar = (product) => {
    setSelectedProduct(product);
    setQuickViewQuantity(1); // Reset quantity for new product
    setQuickViewSize('XS'); // Reset size for new product
    setIsSidebarOpen(true);
  };

  // Handles closing the product quick view sidebar.
  const handleCloseSidebar = () => {
    setIsSidebarOpen(false);
    setSelectedProduct(null);
  };

  // Handles toggling the filter sections (size, price).
  ...
  // Handles selecting a size filter from the left sidebar.
  ...
  // Handles changing the price filter range.
  ...
  // Custom component for the Quick View Sidebar
  ...
  return (
    <div className="text-gray-800 font-sans min-h-screen bg-[#f7f3ed] relative">
      {/* Home / Kurtas moved to very top as per instruction */}
      <div className="w-full">
        <p className="text-sm text-gray-500 text-start w-full px-4 pt-8 sm:px-8">Home / Kurtas</p>
      </div>
      {/* Header with two "KURTAS" */}
      <div className="container mx-auto w-full flex flex-col items-center mb-6">
        <div className="flex flex-col items-center w-full">
          {/* Removed Home / Kurtas from here */}
          <h1 className="text-3xl lg:text-4xl font-light mt-8">KURTAS</h1>
        </div>
      </div>
      ...
      <main className="flex flex-col lg:flex-row p-2 sm:p-6 lg:p-12 space-y-6 lg:space-y-0 lg:space-x-8">
        ...
        {/* Filters Sidebar (hidden on mobile, visible on sm+) */}
              ...
              sortedProducts.map(product => (
                <div key={product.id} className="group product-card bg-white rounded-lg shadow-sm relative">
                  <div className="relative">
                    ...
                    {/* Quick view button centered at the bottom of the image */}
                    <button
