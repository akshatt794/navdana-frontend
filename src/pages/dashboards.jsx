  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (initialData && initialData._id) {
        // Edit
        if (imageFile) {
          await API.put(`/api/v1/category/${initialData._id}`, payload, {
...
function CategoriesContent() {
  ...
  const handleDelete = async (cat) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        await API.delete(`/api/v1/category/${cat._id || cat.id}`);
        fetchCategories();
      } catch (err) {
...
function ProductsContent() {
  ...
  const handleDelete = async (product) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        await API.delete(`/api/v1/product/${product._id || product.id}`);
        fetchProducts();
      } catch (err) {
                    ...
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-gray-200 rounded-lg mr-3 flex items-center justify-center overflow-hidden">
                        {product.images && product.images.length > 0 ? (
                          <img src={product.images[0].url} alt={product.images[0].alt || product.name} className="w-10 h-10 object-cover" />
                        ) : (
